...
class Config:
    ...
    # 批次配置
    batch_size = 1              # 每个GPU的批次大小（micro-batch size）
    gradient_accumulation_steps = 128  # 梯度累积步数，有效批次 = batch_size * gradient_accumulation_steps
    block_size = 1024           # 上下文窗口大小（最大序列长度）
    
    # 训练步数
    max_iters = 10              # 总训练迭代次数
    
    # 优化器配置（AdamW）
    learning_rate = 3e-5        # 学习率（微调时使用较小的学习率）
    ...
    # 学习率调度
    decay_lr = False            # 是否使用学习率衰减（微调时通常用常数学习率）
    ...
    n_head = 12                 # 注意力头数
    n_embd = 768                # 嵌入维度
    dropout = 0.0               # Dropout率（预训练0.0，微调可尝试0.1+）
    ...
    rouge_eval_samples = 10      # 训练时ROUGE评估的样本数（较少避免太慢）
    ...
    compile = False             # 是否使用PyTorch 2.0编译（需要CUDA Capability >= 7.0，P100不支持）
    ...
    max_new_tokens = 20        # 生成时的最大token数
    temperature = 0.8           # 生成温度（1.0=无变化，<1.0=更确定，>1.0=更随机）
    top_k = 20                 # Top-K采样（保留概率最高的K个token）
