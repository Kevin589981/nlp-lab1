...
if torch.cuda.is_available():
    ...
    if n_gpus > 1 and config.use_multi_gpu:
        print(f"\n将使用 {n_gpus} 个GPU进行训练")
        print(f"每个GPU的batch size: {config.batch_size}")
        print(f"总的有效batch size: {config.batch_size * n_gpus * config.gradient_accumulation_steps}")
    else:
        print(f"\n将使用单GPU训练")
else:
    n_gpus = 0
...
def train():
    ...
    print(f"每个GPU的批次大小: {config.batch_size}")
    print(f"GPU数量: {torch.cuda.device_count() if config.use_multi_gpu else 1}")
...
def main():
    ...
    print("\n")
    print("=" * 80)
    print("BART 摘要微调 (多GPU优化)".center(80))
    print("=" * 80)
    print("\n当前配置:")
