...
"""
===================================================================================
GPT-2 文本摘要微调 - 完整教学脚本（多GPU版本）
===================================================================================

本脚本整合了完整的训练和评估流程，支持多GPU并行训练。

主要内容：
1. GPT模型定义（完整的Transformer架构）
2. 数据准备和加载（支持DistributedSampler）
3. 多GPU并行训练（使用DDP）
4. ROUGE评估
5. 生成和测试
...
class GPT(nn.Module):
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        flops_promised = 106e9 # T4 GPU bfloat16 peak flops (修改为T4的性能)
...
class Config:
    ...
    gradient_accumulation_steps = 8  # 梯度累积步数（调整为8，因为有2个GPU，总有效batch=8*8*2=128）
    ...
    dtype = 'float16'  # 使用float16以获得更好的兼容性
    compile = True             # 是否使用PyTorch 2.0编译
...
def setup_ddp():
    ...
    if ddp:
        ...
        print(f"DDP初始化完成: rank={ddp_rank}, local_rank={ddp_local_rank}, world_size={ddp_world_size}")
    else:
        # 单GPU或CPU模式
        ddp_rank = 0
        ddp_local_rank = 0
...
# %%
def get_batch(split, data_dir, ddp=False, ddp_rank=0, ddp_world_size=1):
    """
    获取一个训练批次（支持DDP）
    ...
    try:
        ...
        # 当前epoch结束，重新开始
        # 如果使用DDP，需要设置新的epoch以改变数据分布
        if ddp and hasattr(_dataloaders[split].sampler, 'set_epoch'):
            epoch = getattr(_dataloaders[split], '_epoch', 0) + 1
            _dataloaders[split]._epoch = epoch
            _dataloaders[split].sampler.set_epoch(epoch)
        
        _data_iters[split] = iter(_dataloaders[split])
        x, y = next(_data_iters[split])
    
    # 移动到设备
    device = f'cuda:{ddp_rank}' if ddp else config.device
    if 'cuda' in device:
...
# %%
@torch.no_grad()
def estimate_loss(model, ctx, data_dir, ddp=False, ddp_rank=0, ddp_world_size=1):
    """
    估计训练集和验证集上的损失（支持DDP）
    ...
    for split in ['train', 'val']:
        ...
        for k in range(config.eval_iters):
            X, Y = get_batch(split, data_dir, ddp, ddp_rank, ddp_world_size)
            with ctx:
...
# %%
@torch.no_grad()
def evaluate_rouge_during_training(model, ctx, data_dir, num_samples=3, master_process=True):
    """
    在训练过程中评估ROUGE分数
    ...
    for idx in tqdm(indices, desc="ROUGE评估"):
...
def train():
    ...
    if master_process:
        ...
        print("开始训练...")
        print("=" * 80)
        print(f"DDP状态: {'启用' if ddp else '禁用'}")
        if ddp:
            print(f"总GPU数: {ddp_world_size}")
    ...
    if master_process:
        ...
        print(f"每GPU批次大小: {config.batch_size}")
        print(f"梯度累积步数: {config.gradient_accumulation_steps}")
        print(f"GPU数量: {ddp_world_size}")
        print(f"有效批次大小: {config.batch_size * config.gradient_accumulation_steps * ddp_world_size}")
        print("-" * 80)
    
    X, Y = get_batch('train', data_dir, ddp, ddp_rank, ddp_world_size)
    t0 = time.time()
    local_iter_num = 0
    running_mfu = -1.0
    ...
    while True:
        ...
        # 评估和保存checkpoint（只在主进程）
        if iter_num % config.eval_interval == 0 and master_process:
            losses = estimate_loss(model, ctx, data_dir, ddp, ddp_rank, ddp_world_size)
            ...
            if iter_num > 0 and config.eval_rouge_during_training:
                ...
                rouge_scores = evaluate_rouge_during_training(
                    model, ctx, data_dir, num_samples=config.rouge_eval_samples, master_process=master_process
                )
                if rouge_scores:
...
def main():
    ...
    print("\n")
    print("=" * 80)
    print("GPT-2 摘要微调教学脚本（多GPU版本）".center(80))
    print("=" * 80)
    print("\n当前配置:")
    print(f"  数据集: {config.dataset_path}")
    print(f"  模型初始化: {config.init_from}")
    print(f"  设备: {config.device}")
    print(f"  批次大小（每GPU）: {config.batch_size}")
